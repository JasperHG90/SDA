...
## Subset data
gpg_core <- gpg_core %>%
  dplyr::select(uuid, EmployerSize, DiffMeanHourlyPercent, DiffMedianHourlyPercent, female, male, NumEmp,
         division)
...
for(i in seq_along(1:length(res))) {
  res[[i]] <- required_sample_size(gpg_core, "division", 1000, nrow(gpg_core), 10)
}
...
d <- lapply(1:length(res), function(x) {
  tmp <- res[[x]]$data
  tmp$iteration <- x
  tmp
})
...
```{r, eval=FALSE}
## Look at #employees and GPG by employer size
cm <- gpg_core %>%
  group_by(EmployerSize) %>%
  summarize(emps = sum(NumEmp),
            avgmean = mean(DiffMeanHourlyPercent),
            avgmedian = mean(DiffMedianHourlyPercent),
            avgbonusmean = mean(DiffMeanBonusPercent),
            avgbonusmedian = mean(DiffMedianHourlyPercent))
cm
...
## Weighted mean
weighted.mean(cm$avgmean, cm$emps)
```
