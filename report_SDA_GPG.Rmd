...
One element of the data which we will refer to throughout this report is the Standard Industrial Classification (SIC). This is a widely used system for classifying companies into certain larger industries. Another important variable in the data is the size of an employer ("EmployerSize"), which categorises each company into seven distinct sizes: size not provided, less than 250, 250 to 499, 500 to 999, 1000 to 4999, 5000 to 19,999, and 20,000 or more. We will discuss in detail whether using information on the gender pay gap within these categories can lead to better sampling designs. 
...
``` {r, fig.cap="A histogram of the frequency of different sized mean pay gaps", fig.align="center", fig.show="hold", fig.width=3, fig.height=3}
...
comparedesign <- data.frame(
  "Median wage gap estimate" = c(as.numeric(svymean(~DiffMedianHourlyPercent, swordesign)), 
                               as.numeric(svymean(~DiffMedianHourlyPercent, design=stratdesign_prop, deff=TRUE)), 
                               as.numeric(svymean(~DiffMedianHourlyPercent, design=stratdesign_optim, deff=TRUE))), 
  "SE" = c(as.numeric(SE(svymean(~DiffMedianHourlyPercent, swordesign))), 
                       as.numeric(SE(svymean(~DiffMedianHourlyPercent, design=stratdesign_prop, deff=TRUE))),
                       as.numeric(SE(svymean(~DiffMedianHourlyPercent, design=stratdesign_optim, deff=TRUE)))),
  "Margin of error" = c(as.numeric(SE(svymean(~DiffMedianHourlyPercent, swordesign)))*1.96, 
                       as.numeric(SE(svymean(~DiffMedianHourlyPercent, design=stratdesign_prop, deff=TRUE)))*1.96,
                       as.numeric(SE(svymean(~DiffMedianHourlyPercent, design=stratdesign_optim, deff=TRUE)))*1.96),
  "Design effect" = c(1,
                      as.numeric(deff(svymean(~DiffMedianHourlyPercent, design=stratdesign_prop, deff=TRUE))), 
                               as.numeric(deff(svymean(~DiffMedianHourlyPercent, design=stratdesign_optim, deff=TRUE))))
)         
row.names(comparedesign) <- c("SRS", "PPS", "Optimised")
knitr::kable(comparedesign, caption = "Gender gap estimates under different sampling designs",
             col.names = c("Median wage gap estimate", "SE", "Margin of error", "DEFF"))
```
